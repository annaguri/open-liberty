...
import com.ibm.ws.sip.stack.netty.transport.SipMessageBufferStreamDecoder;
...
public abstract class SipOutboundConnLink extends SipConnLink {
	...
	private Bootstrap bootsrap = new Bootstrap();
	 ...
	 */
	public SipOutboundConnLink(String peerHost, int peerPort, SipInboundChannel sipInboundChannel, Channel channel) {
	...
	public void connect() throws IOException {
		bootsrap.group(workerGroup).channel(NioSocketChannel.class);

		bootsrap.option(ChannelOption.CONNECT_TIMEOUT_MILLIS, s_connectTimeout);
		bootsrap.handler(new ChannelInitializer<SocketChannel>() {
			@Override
		...
		try {
			channelFuture = bootsrap.connect(remoteAddress, localBindAddress).sync();
		} catch (Exception e) {
			e.printStackTrace();
			close();
